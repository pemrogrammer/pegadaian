...
import { filterData, insertData } from "../../store/datatable";
...
const ReactDataTable = ({
  ...
  const stateDatatable = useSelector((state) => state.datatable);
  const dispatch = useDispatch();

  // const [tableData, setTableData] = useState(stateDatatable.tableData[nameTable]);
  const [searchText, setSearchText] = useState("");
  const [rowsPerPageS, setRowsPerPage] = useState(10);
  const [mobileView, setMobileView] = useState();

  useEffect(() => {
    dispatch(filterData({ searchText: searchText, nameTable: nameTable, keyColumns: keyColumns }));
  ...
  return (
    <div className={`dataTables_wrapper dt-bootstrap4 no-footer ${className ? className : ""}`}>
      ...
      <DataTable
        data={stateDatatable.tableData[nameTable]}
        columns={columns}
        className={className}
